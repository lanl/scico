...
def score_fn(score_model: Callable, params: PyTree, x: ArrayLike, t: ArrayLike):
    """Function to apply score model.
...
def Euler_Maruyama_sampler(
    ...
    Returns:
        Samples.
    """
    params_p = jax_utils.replicate(params)
    ...
    for ind, time_step in enumerate(time_steps):
        ...
        mean_x = x + (g**2) * p_score_fn(score_model, params_p, x, batch_time_step) * step_size
        key, step_key = jax.random.split(key)
