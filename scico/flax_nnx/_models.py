...
class DnCNNNet(nnx.Module):
    ...
    def __init__(self,
        ...
        self.blocks = nnx.Sequential(
            nnx.Conv(channels, num_filters, kernel_size=kernel_size, strides=strides, padding="CIRCULAR", use_bias=False, dtype=dtype, kernel_init=kernel_init(), rngs=rngs),
            act,
            *[
                nnx.Sequential(
            ...
            nnx.Conv(num_filters, channels, kernel_size=kernel_size, strides=strides, padding="CIRCULAR", use_bias=False, dtype=dtype, kernel_init=kernel_init(), rngs=rngs),
        )
...
class ResNet(nnx.Module):
    ...
    def __init__(self,
        ...
        self.blocks = nnx.Sequential(
            ConvBNBlock(channels, num_filters, act, kernel_size, strides, kernel_init, dtype, rngs=rngs),
            *[
                nnx.Sequential(
                    ConvBNBlock(num_filters, num_filters, act, kernel_size, strides, kernel_init, dtype, rngs=rngs),
                )
            ...
            ConvBNBlock(num_filters, channels, nnx.identity, kernel_size, strides, kernel_init, dtype, rngs=rngs),
        )
...
class UNet(nnx.Module):
    ...
    def __init__(self,
        ...
        self.start = ConvBNMultiBlock(channels, block_depth, num_filters, act, kernel_size, strides, kernel_init, dtype, rngs=rngs)
  
        # Downsampling path
        self.down = nnx.List([])
        for j in range(depth - 1):
            self.down.append(nnx.Sequential(
                *[
                    ConvBNPoolBlock(2**j * num_filters, 2 * 2**j * num_filters, act, nnx.max_pool, kernel_size, strides, window_shape, kernel_init, dtype, rngs=rngs),
                    ConvBNMultiBlock(2 * 2**j * num_filters, block_depth, 2 * 2**j * num_filters, act, kernel_size, strides, kernel_init, dtype, rngs=rngs)
                ])
            )
        ...
        for j in reversed(range(depth - 1)):
            self.up.append(nnx.List(
                [
                    ConvBNUpsampleBlock(2 * 2**j * num_filters, 2**j * num_filters, act, upfn, kernel_size, strides, kernel_init, dtype, rngs=rngs),
                    ConvBNMultiBlock(2 * 2**j * num_filters, block_depth, 2**j * num_filters, act, kernel_size, strides, kernel_init, dtype, rngs=rngs)
                ])
            )
        
        # Final conv1x1
        ksz_out = (1, 1)
        self.end = nnx.Conv(num_filters, channels, kernel_size=ksz_out, strides=strides, padding="CIRCULAR", use_bias=False, dtype=dtype, kernel_init=kernel_init(), rngs=rngs
        )
    ...
    def __call__(self, x: ArrayLike) -> ArrayLike:
        ...
        for block in self.up:
            ...
            # skip connection
            out = jnp.concatenate((residual[res_ind], out), axis=-1) # channel last
            out = block[1](out)
            res_ind -= 1
