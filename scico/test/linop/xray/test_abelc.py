...
class TestAbelCone:
    ...
    def test_2d(self, num_blocks):
        A = AbelTransformCone(self.x2d.shape, 1e8, 1e8 + 1, num_blocks=num_blocks)
        ya = A(self.x2d)
    ...
    def test_2d_unequal(self, num_blocks):
        x2dc = self.x2d[1:-1]
        A = AbelTransformCone(x2dc.shape, 1e8, 1e8 + 1, num_blocks=num_blocks)
        ya = A(x2dc)
    ...
    def test_3d(self, num_blocks):
        A = AbelTransformCone(self.x3d.shape, 1e8, 1e8 + 1, num_blocks=num_blocks)
        ya = A(self.x3d)
    ...
    def test_3d_unequal(self, num_blocks):
        ...
        A = AbelTransformCone(x3dc.shape, 1e8, 1e8 + 1, num_blocks=num_blocks)
        ya = A(x3dc)
    ...
    def test_2d3d_unequal(self, num_blocks):
        A2d = AbelTransformCone(self.x2d.shape, 5e1, 6e1, num_blocks=num_blocks)
        A3d = AbelTransformCone(self.x3d.shape, 5e1, 6e1, num_blocks=num_blocks)
        y2d = A2d(self.x2d)
        y3d = A3d(self.x3d)
        assert metric.rel_res(y3d, y2d) < 2e-2

    def test_fdk(self):
        A = AbelTransformCone(self.x3d.shape, 1e2, 2e2, num_blocks=1)
        y = A(self.x3d)
        z = A.fdk(y)
