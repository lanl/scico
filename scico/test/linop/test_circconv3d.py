...
class TestCircularConvolve3D:
    ...
    def test_eval(self, axes_shape_spec, input_dtype, jit):
        ...
        A = CircularConvolve3D(h, x_shape, input_dtype=input_dtype, jit=jit)
    ...
    def test_add_sub(self, axes_shape_spec, jit):
        ...
        A = CircularConvolve3D(h, x_shape, input_dtype=input_dtype, jit=jit)
        B = CircularConvolve3D(g, x_shape, input_dtype=input_dtype, jit=jit)
    ...
    def test_matches_convolve(self, input_dtype, jit):
        h, key = randint(minval=0, maxval=3, shape=(3, 4, 1), key=self.key)
        x, key = uniform(minval=0, maxval=1, shape=(5, 4, 1), key=key)

        h = h.astype(input_dtype)
        x = (x <= 0.1).astype(input_dtype)

        # pad to m + n -1
        x_pad = snp.pad(x, ((0, h.shape[0] - 1), (0, h.shape[1] - 1), (0, 0)))

        A = Convolve(h=h, input_shape=x.shape, jit=jit, input_dtype=input_dtype)
        B = CircularConvolve3D(h, input_shape=x_pad.shape, jit=jit, input_dtype=input_dtype)
    ...
    def test_center(self, center, jit):
        x, key = uniform(minval=-1, maxval=1, shape=(16, 1, 1), key=self.key)
        h = snp.array([0.5, 1.0, 0.25])[:, snp.newaxis, snp.newaxis]
        A = CircularConvolve3D(h=h, input_shape=x.shape, h_center=center, jit=jit)
        B = CircularConvolve3D(h=h, input_shape=x.shape, jit=jit)
    ...
    def test_from_operator(self, axes_shape_spec, input_dtype, jit_old_op, jit_new_op):
        ...
        A = CircularConvolve3D(h, x_shape, input_dtype=input_dtype, jit=jit_old_op)

        B = CircularConvolve3D.from_operator(A, jit=jit_new_op)
    ...
    def test_from_operator_block_array(self):
        ...
        with pytest.raises(ValueError):
            CircularConvolve3D.from_operator(H)
