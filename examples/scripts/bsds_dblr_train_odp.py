...
This example demonstrates the training and application of the unrolled optimization with deep priors (ODP) proximal map architecture described in :cite:`diamond-2018-odp` for a deblurring problem.

The original images are part of the [BSDS500 dataset] (http://www.eecs.berkeley.edu/Research/Projects/CS/vision/grouping/BSR/) provided by the Berkeley Segmentation Dataset and Benchmark project.

A class [ODPNet] implements the ODP proximal map architecture, which solves an unrolled iterative optimization problem

    $$ \mathrm{argmin}_{\mathbf{x}} \; \frac{\alpha_k}{2} \| A \mathbf{x} - (\mathbf{y}) \|_2^2 + \frac{1}{2} \| \mathbf{x} - \mathbf{x}^k - \mathbf{x}^{k+1/2} \|_2^2 \;,$$

where $A$ is circular convolution, $\mathbf{y}$ is the blurred image,  $\mathbf{x}^k + \mathbf{x}^{k+1/2}$ is a proximal map-based regularization implemented by a residual convolutional network and $k$ the index of the iteration. Each iteration corresponds to a different stage in the ODP model. The output of the final stage is the deblurred image.
"""
